...
import os 
def dictionary_counter(layered_dict):
...
for machine in machine_list: 
    ...
    for element in response_json.get("UnitInfo", {}).get("Module", []):
        ...
        for nozzle in nozzle_station.get("Nozzle", []):
            if nozzle['JobUse'] == '0' or (nozzle['JobUse'] == '1' and not nozzle['NzlPosition']):
                serial = nozzle["Serial"].split()[0]
                nozzle_info = nozzle_dict.get(serial, serial)
                nozzle_list.append([nozzle_info, [machine_dict[machine], element["@No"], nozzle['@No']]])
...
def update_second(event):
    subkeys = list(counted_data[combobox1.get()].keys())
    combobox2['values'] = subkeys
def show_table():
    key1 = combobox1.get()
    key2 = combobox2.get()
    table_data = counted_data[key1][key2]
...
combobox1 = ttk.Combobox(root, values=list(counted_data.keys()), state="readonly")
combobox1.pack()
